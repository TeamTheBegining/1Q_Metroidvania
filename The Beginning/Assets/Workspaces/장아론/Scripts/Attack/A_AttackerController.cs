...
public class A_AttackerController : CommonEnemyController
{
    ...
    public override void TakeDamage(float damage, GameObject attackObject) // GameObject attackObject 추가
    {
        ...
        // currentHealth는 Base 클래스에서 업데이트됩니다.
        if (currentHealth > 0) // Using Base class currentHealth
        {
            // isDead 플래그는 Base.TakeDamage 호출 후 SetState(Dead)에서 업데이트됩니다.
            // isStunned는 이 메서드 진입 전 체크했습니다.
            Debug.Log("A_Attacker not dead and not stunned. Current Health: " + currentHealth.ToString("F2") + ". Calling PlayHurtAnim."); //  PlayHurtAnim 호출 조건 만족 로그 
            PlayHurtAnim(); // Play hit animation (includes isDead/isStunned check inside PlayHurtAnim)
        }
        else
        {
            Debug.Log("A_Attacker died from damage. Current Health: " + currentHealth.ToString("F2") + ". Death handled by base."); //  데미지로 사망 로그 
    ...
    public void Stun() // Can be called externally (e.g., Player parry)
    {
        ...
        if (isDead || isStunned) return; // protected isDead 사용
    ...
    protected override void Update()
    {
        ...
        if (isDead || isStunned) // protected isDead 사용, isStunned 사용
        {
    ...
    protected override void PlayIdleAnim()
    {
        ...
        if (!isDead && !isStunned && !isPerformingHurtAnimation && animator != null) // protected isDead 사용
    ...
    protected override void PlayDeathAnim()
    {
        ...
        if (animator != null) // isDead 체크는 Base SetState에서 이미 함
    ...
    protected void PlayHurtAnim() // Protected access level
    {
        ...
        if (!isDead && !isStunned && animator != null) // protected isDead 사용
        {
    ...
    public override void OnHurtAnimationEnd() // Base 클래스의 virtual 메서드 오버라이드
    {
        ...
        isPerformingHurtAnimation = false; //  피격 애니메이션 끝 플래그 해제 
        Debug.Log("isPerformingHurtAnimation set to false in OnHurtAnimationEnd."); //  플래그 해제 확인 로그 
    ...
    {
        // Check Base isDead flag
        if (isDead) return; // protected isDead 사용 // Do not enable hitbox if dead
    ...
    {
        // Check Base isDead flag
        if (isDead) return; // protected isDead 사용 // Do not disable hitbox if dead (already disabled in Base SetState(Dead))
